...
export const getGSheetsValue = async (params, { limit = null, offset = null }) => {
  const {
    data: { values },
  } = await request.get(`/values/${params}`);

  const keys = values.shift();

  if (isNumber(limit) && isNumber(offset)) {
    return normalizeSheetsData(values.slice(offset, limit), keys);
  }
...
export const getCourses = async (options = {}) =>
  (await getGSheetsValue('courses!A1:I', options)).map(course => ({
    ...course,
